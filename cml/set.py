...
if __name__ == "__main__":
    ...
    initialMat,matLen,nit,mapping,neigh,output = 'gaussian',128,50,'logistic',[(1,0),(0,1),(-1,0),(0,-1)],False
	...
	for alpha in np.arange(amin,amax,adelta):
		...
		lyap = []
			...
			try:
			   ...
			   maxEigen = np.max(np.fabs(np.real(vals)))
			   csum += np.log(maxEigen)
			   nsum=nsum+1
			   print(str(i)+" - Lyapunov: ",csum/nsum)
			except ArpackError:
			   print("Not convergent for ",i)
		mat[x].append(csum/nsum)
		y=y+1
	x = x+1 
    print(mat)
